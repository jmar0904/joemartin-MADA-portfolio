...
pacman::p_load(pacman, tidyverse, here, lubridate, ggtext, gganimate)
...
The tree model has the same roc_auc value as the previous model. Next, I'll try a random forest to see if I can improve the accuracy. 
...
The two sections below are temporarily hidden. It worked fine in R Studio, but did not knit. 
```{r warning = FALSE}
#rf_best <- rf_res %>%
...
In conclusion, ridge regression was the best predictor of all the models used. However, with a ROC-AUC value of just .45 and an accuracy of 62%, it still isn't strong enough of a model to be useful in predicting the top three finishers in the marble races. At least, this isn't something I would rely on if I were putting money on these races. 
